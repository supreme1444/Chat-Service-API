...
@app.post("/messages")
async def create_message(message: schemas.MessageCreate, db: Session = Depends(auth.get_db),
...
async def get_message_history(recipient_username: str, token: str = Depends(auth.oauth2_scheme),
...
async def get_refresh(recipient_username: str, token: str = Depends(auth.oauth2_scheme),
                      db: Session = Depends(auth.get_db)):
